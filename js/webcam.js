...
function updateResultChart(result_emotion, result_gender) {
    ...
    if(flag==0) //尚未結帳
    {
        indexOfMaxValue = result_emotion.result.reduce((iMax, x, i, arr) => x > arr[iMax] ? i : iMax, 0);
         if(indexOfMaxValue==6)
         {
            // nothing to do
          }
          else if(indexOfMaxValue==3 || indexOfMaxValue==5)
          {
            s_price =s_price + result_emotion.result[indexOfMaxValue]*10
          }
          else
          {
             s_price =s_price - result_emotion.result[indexOfMaxValue]*10
        ...
        //折價多少錢
        if(s_price<5){s_price=5}
        if(s_price>15) {s_price=25}

       // s_price = s_price / 10
      console.log(s_price)
      //  s_price=Math.floor(s_price)
        $("#price").text(s_price)
    }

   // console.log(s_price)
    //console.log(result_gender);
    CHART_GENDER.data.datasets[0].label = '性別: ' + result_gender.label;
    CHART_GENDER.data.datasets[0].data = result_gender.result;
    CHART_GENDER.update(500);

    CHART_EMOTION.data.datasets[0].label = '情緒: ' + result_emotion.label;
    CHART_EMOTION.data.datasets[0].data = result_emotion.result;
    console.log(result_emotion.result)
    if(flag==0)
    CHART_EMOTION.update(1000);
}
